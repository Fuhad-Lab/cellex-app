/**
 * Cellex AI Buyer Assistant - "Cellex"
...
(function() {
    ...
    const CONFIG = {
        // AI chat now goes through our Supabase Edge Function (server-side).
        // The HF Space URL, AI model, and system prompt are all hidden from the frontend.
    ...
    function initFullPage() {
        console.log('[Cellex AI] Full-page mode activated');
    ...
    function showWelcomeMessage() {
        ...
        addMessage('assistant', `<strong>Hi! I'm Cellex, your AI shopping assistant!</strong><br><br>
...
<strong>100% FREE & Open Source!</strong>`);
    ...
    function handleFileSelect(e) {
        ...
        console.log('[Cellex AI] File selected:', file?.name, file?.type, file?.size);
        ...
        img.onload = function() {
            ...
            console.log('[Cellex AI] Image compressed. Original:', file.size, 'bytes. Base64 length:', selectedImage.length);
    ...
    async function updateContext() {
        try {
            console.log('[Cellex AI] Checking login status...');
            console.log('[Cellex AI] window.Cart exists:', !!window.Cart);
            console.log('[Cellex AI] window.EdgeFunctions exists:', !!window.EdgeFunctions);

            if (!window.Cart) {
                console.error('[Cellex AI] Cart module not loaded!');
                await new Promise(resolve => setTimeout(resolve, 500));
                if (!window.Cart) {
                    console.error('[Cellex AI] Cart module still not loaded after waiting');
                    context.user = null;
                    context.cartItems = [];
                    return;
                }
            ...
            console.log('[Cellex AI] User object:', context.user ? `ID: ${context.user.id}` : 'null');

            if (context.user) {
                console.log('[Cellex AI] Fetching cart items for user...');
                const items = await window.Cart.getCartItems();
                context.cartItems = items || [];
                console.log('[Cellex AI] Cart items fetched:', context.cartItems.length);
            } else {
                context.cartItems = [];
                console.log('[Cellex AI] No user logged in - cart empty');
            }
        } catch (e) {
            console.error('[Cellex AI] Error updating context:', e);
            console.error('[Cellex AI] Error stack:', e.stack);
        }
    }
    ...
    async function sendMessage() {
        ...
        if (text && (text.toLowerCase() === 'debug' || text.toLowerCase() === '/debug')) {
            ...
            debugInfo += `window.EdgeFunctions: ${window.EdgeFunctions ? 'Loaded' : 'NOT LOADED'}\n`;
        ...
        console.log('[Cellex AI] sendMessage called. Has image:', !!imageToSend, 'Text:', displayText);
        ...
        try {
            ...
            if (imageToSend) {
                ...
                console.log('[Cellex AI] Image included in request. Base64 length:', imageToSend.length);
            }
            ...
            // Call AI via Supabase Edge Function (server-side).
            // The edge function handles: AI model call, Supabase product search,
            // cart context fetching, login enforcement. Frontend just gets the result.
            const data = await window.EdgeFunctions.aiChat(messageForAI, contextForAI);

            if (statusTimer) clearTimeout(statusTimer);

            if (CONFIG.debug) console.log('[Cellex AI] Response:', data);
            ...
            console.error('[Cellex AI] Error:', error);
            removeLoading();
