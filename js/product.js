...
function updateUIforAuthState(user) {
    ...
    const mobileLogin = document.querySelector('#mobile-auth-links a[href="login.html"]') || document.querySelector('#mobile-auth-links a[href="login.html"]');
    const mobileSignup = document.querySelector('#mobile-auth-links a[href="signup.html"]') || document.querySelector('#mobile-auth-links a[href="signup.html"]');
...
async function startGroupBuy() {
    ...
    if (!currentUser) {
        if (confirm('Please login to start a group buy. Go to login page?')) {
            window.location.href = `login.html?next=${encodeURIComponent('/product.html?id=' + productId)}`;
        }
        return;
    }
...
function openReviewModal() {
    if (!currentUser) {
        if (confirm('Please login to write a review. Go to login page?')) {
            const id = getCurrentProductId();
            window.location.href = `login.html?next=${encodeURIComponent('/product.html?id=' + id)}`;
        }
        return;
    }
