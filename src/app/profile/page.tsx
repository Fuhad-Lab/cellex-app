    ...
    <div className="ig-container min-h-screen pb-24 ig-topbar-offset">
      ...
      <div className="px-4 py-5">
        <div className="flex items-center gap-6">
          <div className="w-20 h-20 rounded-full bg-[#D4AF37] flex items-center justify-center text-black font-bold text-2xl shrink-0 overflow-hidden">
          ...
          <div className="flex-1 grid grid-cols-3 gap-2 text-center">
            ...
            <div>
              ...
              <div className="text-xs text-[#666666]">Wishlist</div>
            </div>
            <div>
              <div className="font-semibold text-base">{isSeller ? 'Yes' : 'No'}</div>
              <div className="text-xs text-[#666666]">Seller</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4">
          <div className="font-semibold text-sm flex items-center gap-1.5">
            ...
            {isSeller && (
              <span className="inline-flex items-center gap-0.5 text-[9px] font-bold bg-[#D4AF37] text-black px-1.5 py-0.5 rounded-full">
        ...
        <button
          ...
          className="w-full mt-4 bg-[#F5F5F5] border border-[#E5E5E5] text-black font-semibold rounded-md py-2 text-sm hover:bg-[#F5F5F5]"
        >
          Edit profile
        </button>
      </div>
      ...
      {/* Tab bar — IG-style */}
      <div className="ig-tab-bar sticky top-[54px] z-20 bg-[#F5F5F5]">
        {tabs.map((t) => {
          const Icon = t.icon;
          return (
            <button
      ...
      {view === 'overview' && (
        <div className="animate-fade-in">
          ...
          <div className="grid grid-cols-3 gap-2 p-4 border-b border-[#E5E5E5]">
            {[
              ...
              const content = (
                <>
                  ...
                  {item.sub && <div className="text-[10px] text-[#666666]">{item.sub}</div>}
                </>
              );
              return item.href ? (
                <Link
                  key={i}
                  href={item.href}
                  className="flex flex-col items-center justify-center p-3 border border-[#E5E5E5] rounded-md hover:bg-[#F5F5F5] transition-colors"
                >
                  {content}
                </Link>
              ) : (
          ...
          <div className="px-4 py-4 border-b border-[#E5E5E5]">
            ...
            {orders.length === 0 ? (
              <div className="text-center py-6">
                <Package className="w-8 h-8 mx-auto text-[#666666] mb-2" />
                <p className="text-xs text-[#666666]">No orders yet</p>
              ...
              <div className="space-y-2">
                {orders.slice(0, 3).map((order) => (
                  <div key={order.id} className="flex items-center gap-3 p-2 border border-[#E5E5E5] rounded-md">
                    <div className="w-10 h-10 rounded bg-[#F5F5F5] flex items-center justify-center shrink-0">
                    ...
                    <span className="text-[10px] font-semibold text-[#666666] px-2 py-0.5 bg-[#F5F5F5] rounded-full">
          ...
          {!isSeller && (
            <Link
              href="/become-seller"
              className="block mx-4 my-4 bg-[#D4AF37] text-black rounded-xl p-4 flex items-center gap-3 hover:bg-[#F5F5F5] transition-colors"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-black/70">Start selling on Cellex today</div>
              </div>
        ...
        <div className="animate-fade-in">
          {orders.length === 0 ? (
            <div className="text-center py-16 px-4">
              <Package className="w-10 h-10 mx-auto text-[#666666] mb-3" />
              <p className="text-sm font-medium text-[#666666]">No orders yet</p>
              <p className="text-xs text-[#666666] mt-1">When you place orders, they'll appear here.</p>
              <Link href="/categories" className="inline-block mt-4 bg-[#D4AF37] text-black text-sm font-semibold px-6 py-2.5 rounded-md">
                Start shopping
              </Link>
            </div>
          ) : (
              ...
              {orders.map((order) => (
                <div key={order.id} className="px-4 py-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] font-semibold px-2 py-1 bg-[#F5F5F5] rounded-full text-[#666666]">
                      {order.status || 'pending'}
                    </span>
                  </div>
                  {/* Order items */}
                  <div className="flex gap-2 overflow-x-auto no-scrollbar">
                    {(order.items || []).slice(0, 4).map((item: any, i: number) => (
                      <div key={i} className="w-12 h-12 rounded bg-[#F5F5F5] overflow-hidden shrink-0">
      ...
      {view === 'store' && isSeller && (
        <div className="animate-fade-in px-4 py-4 space-y-3">
          <Link
            ...
            className="block bg-[#D4AF37] text-black rounded-xl p-4 flex items-center gap-3 hover:bg-[#F5F5F5] transition-colors"
          >
            <div className="w-10 h-10 rounded-full bg-[#F5F5F5] flex items-center justify-center shrink-0">
              <Store className="w-5 h-5" />
            </div>
            <div className="flex-1">
              ...
              <div className="text-xs text-black/70">Manage products, orders, videos</div>
            </div>
            ...
            {[
              ...
              return (
                <Link
                  key={i}
                  href={item.href}
                  className="flex flex-col items-start p-3 border border-[#E5E5E5] rounded-md hover:bg-[#F5F5F5] transition-colors"
                >
        ...
        <div className="animate-fade-in">
            ...
            {!editing ? (
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-[#666666]">{fullName || 'Not set'}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                ...
                <button
                  ...
                  className="w-full mt-2 bg-[#F5F5F5] border border-[#E5E5E5] text-black font-semibold rounded-md py-2 text-sm hover:bg-[#F5F5F5]"
                >
                  Edit profile
                </button>
              </div>
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#666666]">Full name</Label>
                  <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Your name" className={inputClass} />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#666666]">Phone</Label>
                  <Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="08012345678" className={inputClass} />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-[#666666]">Default shipping address</Label>
                ...
                <div className="flex gap-2">
                  <button onClick={saveProfile} disabled={saving} className="flex-1 bg-[#D4AF37] text-black font-semibold rounded-md py-2.5 text-sm hover:bg-[#F5F5F5] disabled:opacity-50">
                    {saving ? 'Saving...' : 'Save'}
                  </button>
                  <button onClick={() => setEditing(false)} className="flex-1 bg-[#F5F5F5] border border-[#E5E5E5] text-black font-semibold rounded-md py-2.5 text-sm hover:bg-[#F5F5F5]">
                    Cancel
                  </button>
                </div>
              </div>
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  <div className="flex-1 pr-3">
                    ...
                    <div className="text-[11px] text-[#666666]">{item.sub}</div>
                  </div>
                  <button
                    onClick={() => item.set(!item.state)}
                    className={`relative w-11 h-6 rounded-full transition-colors ${item.state ? 'bg-[#D4AF37]' : 'bg-[#F5F5F5]'}`}
                    aria-label={`Toggle ${item.label}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-[#F5F5F5] shadow transition-transform ${item.state ? 'translate-x-5' : 'translate-x-0'}`} />
                  </button>
                </div>
              ))}
            </div>
          ...
          <div className="px-4 py-4 border-b border-[#E5E5E5]">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between text-sm">
                <span className="text-[#666666]">Country</span>
          ...
          <div className="px-4 py-4 border-b border-[#E5E5E5]">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2 text-sm py-2">
                ...
                <span className="flex-1 text-[#666666]">HTTP-only cookie auth</span>
                <span className="text-[10px] text-green-600 font-semibold">ACTIVE</span>
              </div>
              <div className="flex items-center gap-2 text-sm py-2">
                <Shield className="w-4 h-4 text-[#666666]" />
                <span className="flex-1 text-[#666666]">Rate limiting on login</span>
                <span className="text-[10px] text-green-600 font-semibold">ACTIVE</span>
              </div>
            </div>
          </div>
          ...
          <div className="px-4 py-6">
            <button
              ...
              className="w-full text-red-400 border border-[#E5E5E5] hover:bg-[#F5F5F5] rounded-md py-2.5 text-sm font-semibold"
            >
              <LogOut className="w-4 h-4 inline mr-2" /> Logout
            </button>
          </div>

          <div className="text-center text-[10px] text-[#666666] pb-4">
