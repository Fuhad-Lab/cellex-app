...
export default function HomePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 space-y-6 pt-4">
      ...
      {/* Meet Top Sellers — horizontal scroll, max 3 + See all card */}
      {sellers.length > 0 && (
        <Section
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pb-4">
        ...
        <Link href="/live" className="bg-primary rounded-2xl p-4 text-center hover:opacity-90 transition-opacity">
        ...
        <Link href="/group-buy" className="brand-gradient rounded-2xl p-4 text-center hover:opacity-90 transition-opacity">
          ...
          <div className="text-xs font-semibold text-white">Group Buys</div>
        </Link>
      ...
      {products.map((p) => (
        <Link key={p.id} href={`/product?id=${p.id}`} className="shrink-0 w-32 sm:w-40">
          <Card className="overflow-hidden border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="p-2">
              <div className="text-sm font-medium text-slate-700 line-clamp-2 h-10 overflow-hidden leading-tight">{p.name}</div>
              <div className="text-base font-bold price mt-1">{formatPrice(p.price)}</div>
            </div>
          </Card>
        </Link>
      ))}
