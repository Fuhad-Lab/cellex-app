 ...
 * Upload Video API
 *
 * Routes through the social Edge Function (op=upload_video) — NO direct
...
export async function POST(request: NextRequest) {
  ...
  const { videoData, productId } = body;
  if (!videoData) {
    return NextResponse.json({ success: false, error: 'videoData required' }, { status: 400 });
  }
  ...
  try {
    const resp = await fetch(`${EDGE_FUNCTIONS_URL}/social`, {
      ...
      body: JSON.stringify({ op: 'upload_video', productId, videoData }),
