        ...
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex items-center border-2 border-neutral-200 rounded-full px-4 py-2 focus-within:border-black transition-colors">
            ...
            <input
              ...
              className="flex-1 bg-transparent outline-none text-base text-black placeholder:text-neutral-400"
            />
          </form>
        </div>
        {/* View toggle */}
        ...
        {view === 'ai' && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex justify-end animate-slide-up">
              <div className="bg-black text-white rounded-2xl rounded-br-md px-4 py-2.5 max-w-[80%]">
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs text-neutral-500 hover:text-black transition-colors animate-fade-in delay-100"
            >
            ...
            {showThoughtProcess && (
              <div className="text-xs text-neutral-400 bg-neutral-50 rounded-lg p-3 space-y-1 animate-slide-up">
            ...
            {aiProducts.length > 0 && !loading && (
              <div className="ml-11 animate-slide-up delay-200">
                <div className="inline-block bg-neutral-100 text-black text-xs font-bold px-2.5 py-1 rounded-md mb-3">
                  {query}
                </div>
          ...
          <div className="animate-fade-in">
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                <VideoIcon className="w-10 h-10 mx-auto text-neutral-300 mb-2" />
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-neutral-200 px-4 py-3">
          <div className="max-w-3xl mx-auto flex items-center gap-2 border-2 border-neutral-200 rounded-full px-4 py-2.5 focus-within:border-black transition-colors">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full bg-black flex items-center justify-center disabled:opacity-30 shrink-0 transition-all hover:scale-105"
            >
...
function SearchResult({ product, index }: { product: Product; index: number }) {
  return (
    <Link href={`/product?id=${product.id}`} className="flex gap-3 group animate-slide-up" style={{ animationDelay: `${index * 50}ms` }}>
      <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-lg overflow-hidden bg-neutral-50 shrink-0 img-zoom">
        {product.image_url ? (
      ...
      <div className="flex-1 min-w-0">
        ...
        <h3 className="text-base font-medium text-black group-hover:underline leading-snug line-clamp-2">
          {product.name}
        </h3>
        <div className="text-lg font-bold price mt-0.5">{formatPrice(product.price)}</div>
        {product.description && (
          <p className="text-sm text-neutral-600 line-clamp-2 mt-0.5 leading-snug">{product.description}</p>
        )}
  ...
  return (
    <Link href="/videos" className="block group animate-scale-in" style={{ animationDelay: `${index * 50}ms` }}>
      <Card className="overflow-hidden border-neutral-200 hover-lift card-transition">
        ...
        <div className="p-2">
          <div className="text-xs font-medium text-black line-clamp-2 h-8 leading-tight">{video.caption || 'Video'}</div>
          <div className="flex items-center gap-1 mt-1">
            <div className="w-4 h-4 rounded-full bg-black flex items-center justify-center text-white text-[8px] font-bold shrink-0">
              {sellerName.charAt(0)}
            </div>
            <span className="text-[10px] text-neutral-500 truncate">@{sellerName}</span>
          </div>
          {product && (
