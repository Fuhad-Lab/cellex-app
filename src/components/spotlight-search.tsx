...
export function SpotlightSearch({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  // Recent searches are kept in memory only (not localStorage).
  // In production, these should be stored in Supabase via an API call.
  // For now, they reset on page refresh — which is fine for UX.
  useEffect(() => {
    // No localStorage — recent searches start empty on each page load
  }, []);
  ...
  const saveRecent = (q: string) => {
    // In-memory only — no localStorage. Will be moved to Supabase API in future.
