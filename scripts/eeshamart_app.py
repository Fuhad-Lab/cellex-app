...
# Chat model - via Pollinations.ai (free, no API key, OpenAI-compatible)
POLLINATIONS_URL = os.environ.get("POLLINATIONS_URL", "https://text.pollinations.ai/openai")
POLLINATIONS_GET_URL = os.environ.get("POLLINATIONS_GET_URL", "https://text.pollinations.ai")
...
AI_TIMEOUT = float(os.environ.get("AI_TIMEOUT", "30"))  # seconds for Pollinations call

print(
    "Config: "
    f"SUPABASE_URL={'set' if SUPABASE_URL else 'MISSING'}, "
    f"SUPABASE_KEY={'set' if SUPABASE_KEY else 'MISSING'}, "
    f"AI_MODEL={AI_MODEL} (Pollinations.ai), "
...
async def load():
    ...
    print(f"Chat model: {AI_MODEL} via HF Router (remote, no local load)")
...
async def _call_hf_router(messages: List[Dict]) -> Optional[Dict]:
    """Single Pollinations.ai call. Returns parsed {reply, calls} dict, or None on failure."""
    payload = {
        "model": AI_MODEL,
        "messages": messages,
    ...
    try:
        ...
        print(f"Pollinations request error: {e}")
        return None

    if resp.status_code != 200:
        print(f"Pollinations POST HTTP {resp.status_code}, trying GET fallback...")
        # Fallback to GET endpoint
        try:
            full_prompt = "\n".join([f"{m['role']}: {m['content']}" for m in messages])
            encoded = urllib.parse.quote(full_prompt)
            async with httpx.AsyncClient(timeout=AI_TIMEOUT) as client:
                get_resp = await client.get(f"{POLLINATIONS_GET_URL}/{encoded}?model=openai")
            if get_resp.status_code == 200:
                text = get_resp.text.strip()
                if text and "<!DOCTYPE" not in text:
                    return {"reply": text, "calls": []}
        except Exception as e2:
            print(f"Pollinations GET fallback error: {e2}")
        return None

    try:
        data = resp.json()
    except Exception as e:
        print(f"Pollinations JSON parse error: {e}")
        return None
